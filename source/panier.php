...
if ($nbRefPanier == 0) {
	...
	echo '<script>alert("Vous avez rentré une quantité négative. Cela peut être pratique pour rattraper une erreur dans un achat précédent. Mais si vous vous êtes trompé, supprimez la référence de votre panier.");</script>';
...
<form id="formulaire" method="post" action="payer.php">
	...
	<table>
	    ...
	    ?>
		<tr>
		    <td width="78%"><?php echo htmlspecialchars($_SESSION['panier']['nomReference'][$i]);?></td>
		    <td width="9%"><?php echo htmlspecialchars($_SESSION['panier']['qteReference'][$i]);?></td>
